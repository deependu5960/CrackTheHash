...
def check_database_cache(hash_value):
    """Checks your Neon database via Vercel to see if the hash is already cracked."""
    try:
        ...
        print(f"[*] Database cache bypass (Server offline or timeout): {e}")
    return {"found": False}

def save_to_database_cache(hash_value, algo, plain_text):
    """Sends a newly cracked hash to your Neon database so you never have to crack it again."""
    try:
        payload = {
        ...
        print("[+] Successfully saved hash mapping to central Neon DB!")
    except Exception as e:
        print(f"[*] Failed to save to database cache: {e}")
